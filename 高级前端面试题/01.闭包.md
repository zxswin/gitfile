...
```js
/**
 * js变量（不包括函数）的作用域是函数级的，也就是说定义变量的整个函数区域都可以访问到它，
 * 即使这个变量是否定义在while、if代码块中也可以。
 */
'use strict';
function test() {
  var i = 2;
...
function foo() {
  ...
  var y = 'this is y'; // 如果将var y = 'this is y';这一行注释掉则会报错
}
foo();
...
```js
/**
 * 当函数被调用的时候，调用函数的那个对象会被传递到执行上下文中，成为this的值。
 * 函数 this 的指向与这个函数定义在哪个对象中无关，只与调用这个函数的对象有关，this是在调用时确定的，而不是定义时。
 *
 * 在全局上下文this就指向window对象。
 * 在构造函数中，this指向正在构造的对象。
 * 通过apply或call调用函数的时候，或者调用通过bind得到的函数的时候，程序员可以自己决定this的指向，this就指向第一个参数
 */
'use strict';
var obj = {
  a: 111,
...
/**
 * this会指向undefined——因为没有任何一个对象调用它
 * 如果是在非严格模式下是Window {…}
 */
fn_ref(); // undefined
...
即使函数已经执行完毕，在执行期间创建的变量也不会被销毁，因此每运行一次函数就会在内存中留下一组变量。
js当然会有垃圾回收机制，不过如果它发现你正在使用闭包，则不会清理可能会用到的变量
```
...
window.onload = function() {  //  函数1
    ...
    // i在同一个环境中被声明的（都是在执行函数1的环境） 当循环结束的时候停留在3
...
/**
  * 四个onclick函数必须有自己“声明环境”！既然要产生4个“环境”
  */
<ol>
    <li>第一项</li>
    <li>第二项</li>
...
window.onload = function() {  //  函数1
    ...
    for (var i = 0; i < lis.length; i++) {
        /**
          * 函数被自执行 每执行一次产生一个闭包环境 private_i参数变量被记住了
          */
...
```bash
用一个闭包函数包裹模块的代码，将不需要暴露的变量隐藏起来（好处是不会污染全局变量空间），
将别人要调用的方法return出去，就可以实现模块化了。实际上Node.js就是这么做的
