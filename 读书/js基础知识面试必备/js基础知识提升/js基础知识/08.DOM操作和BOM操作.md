...
- querySelectorAll 返回指定选择器的所以对象集合
...
/**
 * 2.了解跟this有非常相似的属性currentTargtet
 */
...
```ts
/**
 * BOM即浏览器对象模型 它提供两独立与内容而与浏览器窗口进行交互的对象
 * 其核心的对象是window
 * DOM是W3C标准规范
 * BOM缺乏标准
 * BOM比DOM更大 它包含DOM
 ...
 * 它有很多属性 比较常用的是userAgent 该属性可以返回由客户机发生服务器的user-agent头部的值
 *
 ...
 * 4.element.offsetWidth 返回自身包括padding 边框 内容去的宽度 返回的数值不带单位
 ...
 * 3.element.scrollWidth 返回自身的实际的宽度 不含边框 返回的数值不带单位(包含内容溢出的部分,clientHeight是不包含的)
