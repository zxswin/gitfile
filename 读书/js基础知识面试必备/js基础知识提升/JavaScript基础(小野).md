 ...
 * 同时第1位存储GO 查找变量是到a函数存储的作用域链中虫顶端开始依次向下查找
 */
 ...
 * 这个test1的AO被没有被销毁，只是把线剪断了,test2的作用域链还连着的
 */
function test1() {
  function test2() {}
  var a = 1;
...
/**
 * test3执行,test2的作用域链增加自己的AO,当打印a的时候，再自己的AO上没有查找到,则向test1的AO查找,再次执行test3时,实际操作的仍然是原来test1的AO
 */
...
/**
 * 数组
 * 修改原数组的方法 push/unshift/pop/shift/reverse/slice/sort
  */
// 数组后面打了逗号其实和没打一样
var arr = [1, 3, 5, 7];
