...
```html
<pre>这是一个原样输出标签，空格和换行都会元样输出</pre>
...
/** 选中第一个子元素 后代子元素下面的第一个子元素也会被选中  */
article:nth-child(1) {
}
/** 只选中自己下面的第一个子元素 子元素下面的不会被选中  */
article > :nth-child(1) {
}
/** 选中第奇数个子元素  */
...
article {
  width: 500px;
  height: 500px;
  background: radial-gradient(at 0% 50%, red, green, blue) // 通过at 0% 50%设置渐变的开始位置 也可以是方向值;
}
...
main::after {
  ...
  transform-origin: bottom;
  transition-timing-function: steps(60, start); // 从第一步开始动一共动3步
